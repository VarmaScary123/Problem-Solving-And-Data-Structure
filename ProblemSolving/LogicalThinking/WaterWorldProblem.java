...
Input 2 3 C1 100 300 C2 150 300 C3 100 100 3 F_C1 F_C2 C2_C3 Output 1100 In the above on day 1, C1 gets 300, C2 gets 300, C3 gets 100 End of day 1, C1 would have 200, C2 would have 150, C3 would have 0
...
public class WaterWorldProblem {
    public static void main(String arg[]) {
        ...
        Map<String, Integer> variance = new HashMap<>();
        Map<String, Integer> reqWater = new HashMap<>();

        while (numberOfClusters-- > 0) {
            String clusterName = sc.next();
            int waterRequired = sc.nextInt();
            int totalCapacity = sc.nextInt();

            variance.put(clusterName, (totalCapacity / waterRequired));
            reqWater.put(clusterName, totalCapacity);
            minimumWater += totalCapacity;
        }

        int links = sc.nextInt();

        while (links-- > 0) {
            String linkPipe[] = sc.next().split("_");
            reqWater.put(linkPipe[1], reqWater.getOrDefault(linkPipe[1], 0) + reqWater.getOrDefault(linkPipe[0], 0));
        }

        int mul = 1;

        for (String s : variance.keySet()) {
            int temp = variance.get(s) * mul;

            if (temp < days) {
                minimumWater += (reqWater.get(s) * (days - temp));
                mul++;
            }
        }
        System.out.println(minimumWater);
        sc.close();
    }
}